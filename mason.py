...
total_budget = seed + debt # 9,000만 원 (유동 반영)

# [후보 종목 리스트] TNA와 LABU를 지독하게 최우선 배치했습니다! ㅋㅋㅋ
...
if holdings_str:
    ...
    for h in holdings:
        try:
            ...
            avg_price = float(parts[1]) # 평단 (소수점 보관)
            quantity = float(parts[2])  # 보유 수량 (제자리 고정!)
